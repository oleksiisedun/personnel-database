...
The template also carries a `mode` variable, set by the server function that opens the dialog (`template.mode = ...` before `.evaluate()`) and surfaced to the client as `const INITIAL_MODE = <?!= JSON.stringify(mode); ?>;` (must use the unescaped `<?!=` form — `JSON.stringify()`'s quotes would otherwise be HTML-escaped and corrupt the token). `openWebEditor()` sets `mode = null` (the normal list-view editor); `openPhotoExport()` sets `mode = 'photoExport'`, which `init()` in `WebEditor.js.html` checks first and, if matched, skips the normal `getSchemaAndData()` bootstrap entirely and drives a different flow instead (see "Photo export for S-КАДР" below). Every server function that calls `HtmlService.createTemplateFromFile('WebEditor')` must set `template.mode` explicitly — an unset scriptlet variable throws `ReferenceError` when referenced.
...
`getSchemaAndData()` includes the result as `actualPersonnelNames: string[]|null` in its return value. The client enables the **"Actual personnel"** toolbar checkbox only when the array is non-null and non-empty; otherwise the checkbox stays disabled. When the checkbox is checked, `applyFilters()` additionally requires that `row.values[0]` (first column = full name) is present in `actualPersonnelNames`, via the shared `isActualPersonnelRow(row)` helper. The filter composes with all existing column text filters and the regex toggle.

The checkbox defaults to **checked** whenever it's enabled — both on initial load (`onDataLoaded()`) and whenever `resetListState()` runs — so the Actual Personnel filter is applied unless a user explicitly unchecks it.

`getSelectedRows()`/`getSelectedRowEntries()` (see "Document export" below) also apply `isActualPersonnelRow()` when `filterActualPersonnel` is on, excluding any checked row not on the list even if it was selected under a different filter state. This is a deliberate exception to `getSelectedRowEntries()`'s general "selection persists across filter changes" behavior (see below) — the Actual Personnel checkbox is a correctness gate (which people should ever appear in an export/move/delete), not a display-only convenience filter like column text search, so a stale off-list selection must never silently leak into an exported file.
...
`_makeSheetDataLoader(localSs)` (Export.js) is the per-spreadsheet cache/loader factory shared by `_exportDoc()`, `exportXLSX()`, and `startPhotoExport()` (below), so a Master Mode export spanning multiple sources still reads each distinct spreadsheet at most once per call. New export-style functions that need "read each selected row's source spreadsheet, cached" should reuse this instead of re-inlining the cache.

### Photo export for S-КАДР (`Export.js`)

The "Export Photos for S-КАДР" menu item (added by `onOpen()` alongside "Open Web Editor" etc.) is only shown when Master Mode is on (`getMasterMode()`), since it's meaningless for a spreadsheet with no other sources to aggregate across — the only menu item currently gated this way. Clicking it calls `openPhotoExport()` (`Code.js`), which opens the *same* `WebEditor` modal as `openWebEditor()` but with `template.mode = 'photoExport'` (see "How the HTML template works" above) instead of building a separate dialog — the client's `init()` detects this mode and immediately drives the shared `#export-overlay` progress UI instead of the normal list-view bootstrap.

Unlike every other export path, this one has no manual checkbox selection: it sweeps **every** row in the Actual Personnel list (`getActualPersonnelNames()`) across the local `Database` sheet and every Master Mode source, rather than operating on `getSelectedRowEntries()`. Because that scan can span many spreadsheets and Apps Script has a 6-minute execution ceiling, the work is split into two phases:

1. **Discovery** — `startPhotoExport()` creates the destination Drive folder (named `"Photos for S-КАДР DD.MM.YYYY"`, `PHOTO_EXPORT_FOLDER_PREFIX` + `formatDateDDMMYYYY()`, inside `Handbook!M13`/`EXPORT_FOLDER_CELL`), then walks local + every `getMasterSources()` ID **in that fixed order** — deliberately not the concurrent worker-pool pattern `queueMasterSourceFetch()` uses client-side, since duplicate-ID resolution below needs a reproducible processing order. For each source (via `_makeSheetDataLoader()`), it filters rows to the Actual Personnel set and reads the `Фото`/`S-КАДР ID` columns (`COL_PHOTO`/`COL_CARD_ID` in `Config.js`, matched via `getFieldByPattern()`). A person missing either value is skipped and recorded with a reason; if the same S-КАДР ID has already been seen (e.g. duplicated across two master sources) the first occurrence wins and later ones are skipped as duplicates. This phase fetches no image bytes — only sheet values — so the complete eligible list, and therefore duplicate detection, is known before any Drive I/O starts. Like `exportXLSX()` (and unlike `_exportDoc()`), it has no continuation: a half-scanned personnel list has no standalone value, so it throws if `EXPORT_TIME_LIMIT_MS` is exceeded rather than returning partial results.
2. **Copy/convert** — `copyPhotosBatch(folderId, entries)` processes one batch of the eligible list, following the exact `{results, remaining}` time-boxed continuation `_exportDoc()` uses (plus a third `skipped` field, new here since per-item failures need to be surfaced to the user rather than silently swallowed). For each person it fetches the **full-resolution** original blob (`DriveApp.getFileById(fileId).getBlob()` — deliberately not `_getExportImageBlob()`'s 800px thumbnail path used for Doc exports, since this feed needs source-quality photos) and force-converts it to JPEG via `blob.getAs('image/jpeg')` (not used anywhere else in the codebase), saving it as `{S-КАДР ID}.jpg`. A blob fetch or conversion failure becomes a skip entry rather than aborting the run.

Client-side (`WebEditor.js.html`), `startPhotoExportFlow()` kicks off discovery, `onPhotoDiscoverySuccess()` starts (or, if nothing is eligible, skips) the batch loop, `runPhotoCopyBatch()` recurses on `response.remaining` exactly like `startExportProgress()`'s `fire()`, and `finishPhotoExport()` renders the final message with a link to the destination folder plus the accumulated skip/duplicate list via `renderPhotoSkipLog()` into `#export-skip-log` (`.log-list`, shared with the Move dialog's `#move-log`). Because this mode never calls `getSchemaAndData()`, `schema` stays `null` for the life of the dialog — none of these functions read it. `#btn-export-close` closes the whole dialog (`google.script.host.close()`) in this mode instead of just hiding the overlay, since there's no list view to return to.

### Award import from S-КАДР (`Import.js`)

The "Import awards from S-КАДР" menu item (added by `onOpen()` alongside "Export Photos for S-КАДР", inside the same `getMasterMode()` gate) calls `importAwards()`. Unlike every other Master Mode feature, this one uses plain `ui.prompt()`/`ui.alert()` instead of the `WebEditor` modal — the operation is a synchronous sheet-to-sheet scan with no per-row Drive I/O, so it comfortably finishes in one execution without needing the progress-overlay/batching infrastructure Photo Export requires.

`importAwards()` prompts for the source sheet's full URL, resolves the spreadsheet via `parseDriveId()` + `openSpreadsheetSafely()`, and picks the exact tab via `parseGidFromUrl(url)` (Utils.js) — matching `?gid=...`/`#gid=...` in the pasted URL against `sheet.getSheetId()` — falling back to the first tab if the URL has no gid. The import sheet's layout is fixed by column position, not header text (`AWARDS_IMPORT_ID_COL`/`_NAME_COL`/`_ORDER_NUMBER_COL`/`_ORDER_DATE_COL` in `Config.js`, stored as A1 column letters — `'A'`/`'F'`/`'G'`/`'H'` — data from row 2). `_groupAwardsById()` converts each letter to a 0-based array index once via `columnLetterToIndex()` (Utils.js) before indexing into the raw `getValues()` rows.

`_groupAwardsById()` reads that fixed layout into `{ id: [name, orderNumber, orderDate][] }`, skipping rows with no ID or no award name, capitalizing the award name's first letter, and cleaning the order number with `AWARDS_ORDER_NUMBER_CLEAN_REGEX` (`/\/\d+|[\W]+/g` — `\W` is ASCII-only, so this strips Cyrillic text/spaces/`№` as well as a trailing `/2022`-style suffix, e.g. `"Указ Президента України № 559/2022"` → `"559"`).

Matching searches the local `Database` sheet plus every `getMasterSources()` spreadsheet (`_collectAwardTargetSources()` → `_addAwardSource()` per source), locating each source's `S-КАДР ID`/`Нагороди` columns via `findColumnIndex()` (Utils.js — also now used by `fixPhoneNumbers()`/`fixFullNames()`, which inlined the same lookup before this feature's third use crossed the extraction threshold) and precomputing an `id → row index` map per source. `_applyAwards()` resolves each imported ID to the first source that has it (local first, then `getMasterSources()` order — same "first occurrence wins" convention as Photo Export's duplicate-ID handling), decodes the existing `Нагороди` cell via `_parseSubTable()` (`Export.js`), appends new entries while skipping ones that exactly duplicate an existing `[name, number, date]` triple, and writes the merged cell back via the new `_encodeSubTable()` (`Export.js`, the inverse of `_parseSubTable()`). `_parseSubTable()`'s separators were extracted into `Config.js`'s `TABLE_FIELD_SEP`/`TABLE_ROW_SEP` constants as part of this (behavior-preserving) — the client (`WebEditor.js.html`) still declares its own copies of the same two literals since it runs in a separate HTML-embedded script, not this server runtime.

The final `ui.alert()` shows counts (people updated, entries added, duplicates skipped, IDs not found) plus up to `AWARDS_IMPORT_NOT_FOUND_DISPLAY_LIMIT` not-found IDs by name; the full per-ID log is written via `Logger.log()` for audit purposes, since an uncapped alert dialog isn't scrollable.
...
**Sheet cell/column/row references in `Config.js` use A1 notation**, matching the existing Handbook range constants (`MASTER_MODE_CELL = 'M2'`, `HANDBOOK_UNIT_RANGE = 'D17:D40'`, `MASTER_MODE_SOURCES_RANGE = 'N2:N'`, etc.) — a 0-based numeric column index is not self-describing against the sheet's actual layout the way a letter is. This applies to single-column constants too (e.g. `AWARDS_IMPORT_ID_COL = 'A'`, not `= 0`). When code needs a 0-based array index to read a raw `getValues()` row (e.g. `row[idColIndex]`), convert the letter once via `columnLetterToIndex(letter)` (`Utils.js`) rather than storing the numeric index directly in `Config.js`.
