...
- **Matching** searches the local `Database` plus every `getMasterSources()` spreadsheet (`_collectAwardTargetSources()` → `_addAwardSource()`), locating `S-КАДР ID`/`Нагороди` via `findColumnIndex()` (`SchemaHelpers.js`, also used by `fixPhoneNumbers()`/`fixFullNames()` in `DataFixes.js`) and precomputing an `id → row index` map per source.
...
- `_groupAwardsById()` and `_buildImportSummary()` touch no Apps Script services and are unit-tested in `tests/import.test.js`; **keep new grouping/summary logic here pure and testable** the same way.
